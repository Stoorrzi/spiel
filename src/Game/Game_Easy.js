...
export default function Game_Easy() {
    ...
    return (
      <div>
        ...
        <div className="D14">
          <div className="D13">
            <h1 className="H12" style={{ fontSize: `calc(70px - ${title.length * 0.33}px)` }} >{title}</h1>
            <p className="P12" style={{ fontSize: `calc(50px - ${fragen[0].length * 0.035}px)` }} >{fragen[0]}</p>
          </div>
        </div>
        <button
          className="B11"
  ...
  if (antwort) {
    return (
      <div className="D15">
        <p1 className="H12" style={{ fontSize: `calc(70px - ${title.length * 0.4}px)` }} >{fragen[0]}</p1>
        <div className="D16" style={{ height: '450px' }}>
          {antworten[0].map((answer, index) => (
            <button key={index} className="B13" onClick={() => { checkAnswer(index + 1, setStory(true)) }}>{answer} </button>
          ))}
        </div>
